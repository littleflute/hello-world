...
public:
    ...
    Status GetNearestColor(IN OUT GdiPlusColor* color) const
    {
        if (color == NULL)
        {
    ...
    Status Clear(IN const GdiPlusColor &color)
    {
    ...
    BOOL IsVisible(IN INT x,
                   IN INT y) const
    {
        return IsVisible(PointI(x,y));
    }

    BOOL IsVisible(IN const PointI& point) const
    {
        BOOL booln = FALSE;
