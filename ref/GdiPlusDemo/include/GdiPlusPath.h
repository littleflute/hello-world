...
public:
    ...
    GraphicsPath(IN const PointI* points,
                 IN const BYTE* types,
                 IN INT count,
                 IN FillMode fillMode = FillModeAlternate)
    {
    ...
    Status AddLine(IN const PointI& pt1, 
                   IN const PointI& pt2)
    {
    ...
    Status AddLines(IN const PointI* points, 
                    IN INT count)
    {
    ...
    Status AddBezier(IN const PointI& pt1, 
                     IN const PointI& pt2,
                     IN const PointI& pt3, 
                     IN const PointI& pt4)
    {
    ...
    Status AddBeziers(IN const PointI* points,
                      IN INT count)
    {
    ...
    Status AddCurve(IN const PointI* points, 
                    IN INT count)
    {
    ...
    Status AddClosedCurve(IN const PointI* points, 
                          IN INT count)
    {
    ...
    Status AddPolygon(IN const PointI* points, 
                      IN INT count)
    {
    ...
    Status GetPathPoints(OUT PointI* points, 
                         IN INT count) const
    {
    ...
    BOOL IsVisible(IN const PointI& point,
                   IN const Graphics* g = NULL) const
    {
        return IsVisible(point.X, point.Y, g);
    }
    ...
    BOOL IsOutlineVisible(IN const PointI& point,
                          IN const Pen* pen, 
                          IN const Graphics* g = NULL) const
    {
        return IsOutlineVisible(point.X, point.Y, pen, g);
...
public:
    ...
    Status GetCenterColor(OUT GdiPlusColor* color) const
    {
        ARGB argb;
    ...
    Status SetCenterColor(IN const GdiPlusColor& color)
    {
    ...
    Status GetSurroundColors(OUT GdiPlusColor* colors, 
    ...
    Status SetSurroundColors(IN const GdiPlusColor* colors, 
                             IN OUT INT* count)
    {
        if(colors == NULL || count == NULL)
        {
    ...
    Status GetCenterPoint(OUT PointI* point) const
    {
    ...
    Status SetCenterPoint(IN const PointI& point)
    {
    ...
    Status SetInterpolationColors(IN const GdiPlusColor* presetColors,
                                  IN const REAL* blendPositions, 
                                  IN INT count)
    {
    ...
    Status GetInterpolationColors(OUT GdiPlusColor* presetColors,
                                  OUT REAL* blendPositions, 
                                  IN INT count) const
    {
        if ((count <= 0) || !presetColors) 
        ...
        for(INT i = 0; i < count; i++)
        {
            presetColors[i] = GdiPlusColor(argbs[i]);
        }
        delete [] argbs;
